...
resource "aws_lambda_function" "link_generator" {
  ...
  handler  = "main"
  runtime  = "go1.x"
  filename = "../lambdas/link-generator/main.zip"

  role = aws_iam_role.link_generator_role.arn

  source_code_hash = filebase64sha256("../lambdas/link-generator/main.zip")
...
resource "aws_lambda_function" "link_resolver" {
  ...
  handler  = "main"
  runtime  = "go1.x"
  filename = "../lambdas/link-resolver/main.zip"

  role = aws_iam_role.link_resolver_role.arn

  source_code_hash = filebase64sha256("../lambdas/link-resolver/main.zip")
